...
metadata:
  ...
  namespace: maria
  annotations:
    repository.flux.weave.works/custom: spec.values.mariadb.customImage
    tag.flux.weave.works/custom: spec.values.mariadb.customTag
spec:
  chart:
    repository: https://example.com/charts
    name: mariadb
...
var case14ImageMap = kresource.ContainerImageMap{
	Repository: "spec.values.mariadb.customImage",
	Tag:        "spec.values.mariadb.customTag",
}
